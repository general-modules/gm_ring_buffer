/**
 * @file      example_ring_buffer.c
 * @brief     环形缓冲区模块示例代码源文件
 * @author    huenrong (sgyhy1028@outlook.com)
 * @date      2026-02-16 16:26:13
 *
...
static gm_ring_buffer_t *s_test_ring_buffer = NULL;
...
static void *consumer_thread(void *arg)
{
    ...
    while (true)
    {
        ...
        if (s_read_mode == 'b')
        {
            ssize_t ret = gm_ring_buffer_read(s_test_ring_buffer, read_data, sizeof(read_data));
            if (ret > 0)
            {
                current_timestamp = 0;
                gm_time_get_current_timestamp_ms(&current_timestamp);
        ...
        {
            ssize_t ret = gm_ring_buffer_read_with_timeout(s_test_ring_buffer, read_data, sizeof(read_data), 600);
            if (ret > 0)
            {
                current_timestamp = 0;
                gm_time_get_current_timestamp_ms(&current_timestamp);
...
int main(int argc, char *argv[])
{
    ...
    s_test_ring_buffer = gm_ring_buffer_create();
    ...
    int ret = gm_ring_buffer_init(s_test_ring_buffer, 1024);
    if (ret != 0)
    {
    ...
    while (true)
    {
        ...
        ssize_t ret = gm_ring_buffer_write(s_test_ring_buffer, s_write_data, write_len);
        if (ret != write_len)
        {
